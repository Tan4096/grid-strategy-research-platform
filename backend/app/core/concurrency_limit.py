...
def _enabled() -> bool:
    return _truthy(os.getenv("APP_CONCURRENCY_LIMIT_ENABLED"), default=True)


def _per_subject_limit() -> int:
    return max(1, int(os.getenv("APP_CONCURRENCY_LIMIT_PER_SUBJECT", "2")))


def _per_ip_limit() -> int:
    return max(1, int(os.getenv("APP_CONCURRENCY_LIMIT_PER_IP", "4")))


def _global_limit() -> int:
    return max(1, int(os.getenv("APP_CONCURRENCY_LIMIT_GLOBAL", "64")))


def _ttl_seconds() -> int:
    return max(30, int(os.getenv("APP_CONCURRENCY_LIMIT_TTL_SECONDS", "300")))
    ...
    try:
        ...
        for key in touched_keys:
            try:
                ...
                continue
        return False, 1, ConcurrencyLease(
            scope=scope,
            backend="redis",
            subject_key=None,
            ip_key=None,
...
def _cleanup_memory_locked(now: float) -> None:
    ...
    stale_keys = [
        key
        for key, counter in _MEMORY_COUNTERS.items()
        if counter.value <= 0 and (now - counter.updated_at) > ttl
    ]
...
def _memory_try_acquire(
    *,
    ...
    with _LOCK:
        ...
        allowed = (
            current_subject <= subject_limit
            and current_ip <= ip_limit
            and current_global <= global_limit
        )
        if not allowed:
...
def acquire_concurrency_slot(
    ...
    allowed, retry_after, lease = _memory_try_acquire(
        scope=scope,
        subject_key=subject_key,
        ip_key=ip_key,
        global_key=global_key,
    )
    return allowed, retry_after, scope, lease
...
def release_concurrency_slot(lease: ConcurrencyLease | None) -> None:
    ...
    keys = [lease.subject_key, lease.ip_key, lease.global_key]
